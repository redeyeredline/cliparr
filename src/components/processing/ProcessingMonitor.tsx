          ...
          <AnimatePresence>
            {activeProcesses.map((job) => {
              ...
              const audioPercent =
                ...
                job.audioPercent ??
                (job.progress < 50 ? job.progress * 2 : 100) ??
                0;
              const chunkPercent =
                realTimeProgress?.progress ??
                job.chunkPercent ??
                (job.progress >= 50 ? (job.progress - 50) * 2 : 0) ??
                0;
              const overall = Math.round(audioPercent * AUDIO_WEIGHT + chunkPercent * CHUNK_WEIGHT);
              const fileName = getFileName(
                job.filePath ||
                  job.currentFile?.filePath ||
                  realTimeProgress?.currentFile?.filePath ||
                  '',
              );
              ...
              return (
                ...
                >
                  <h3 className="font-semibold mb-2 text-base text-white">{fileName}</h3>
