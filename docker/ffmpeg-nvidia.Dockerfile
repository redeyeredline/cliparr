...
#  from the NVIDIA Dockerfile so regular Cliparr builds are fast.
# -----------------------------------------------------------------------------
...
LABEL org.opencontainers.image.title="FFmpeg ${FFMPEG_VERSION} static build (NVIDIA)" \
      org.opencontainers.image.description="Prebuilt FFmpeg binaries for use in Cliparr NVIDIA builds" \
