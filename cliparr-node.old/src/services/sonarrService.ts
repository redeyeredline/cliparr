...
class SonarrService {
  ...
  async fetchSeries(): Promise<SonarrSeries[]> {
    try {
      ...
      const sonarrError = error as SonarrError;
      console.error('Error fetching series:', sonarrError.message);
      throw new Error(`Failed to fetch series: ${sonarrError.message}`);
    }
  }
