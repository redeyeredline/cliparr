// This file has been moved from src/components/SonarrTest.jsx to src/frontend/components/SonarrTest.jsx for correct frontend import resolution. 
...
const SonarrTest = () => {
    ...
    const testConnection = async () => {
        setLoading(true);
        setError(null);
        setSampleEpisode(null);
        setSeries([]);
        setConnectionStatus('Testing...');
        console.log('--- Sonarr API Test: Starting ---');
        try {
            // Fetch series
            console.log('Fetching series from /api/sonarr/series...');
            const seriesResponse = await axios.get('/api/sonarr/series');
            console.log('Series API response:', seriesResponse.data);
            if (Array.isArray(seriesResponse.data)) {
                setSeries(seriesResponse.data);
                setConnectionStatus('Connected to Sonarr API');
            } else {
                setSeries([]);
                setConnectionStatus('Unexpected response: not an array');
                setError('Series API did not return an array.');
                return;
            }
            ...
            if (seriesResponse.data.length > 0) {
                ...
                console.log('Fetching episodes for show:', firstShow);
                ...
                if (Array.isArray(episodesResponse.data) && episodesResponse.data.length > 0) {
                    setSampleEpisode({
                    ...
                    setError('No episodes found for the first show or episodes response was not an array.');
                }
            } else {
                setError('No series found in Sonarr.');
            }
        } catch (err) {
            ...
            setLoading(false);
            console.log('--- Sonarr API Test: Finished ---');
        }
    };
            ...
            {sampleEpisode && (
                <div className="mt-4 bg-white rounded-lg shadow p-4">
                    ...
                    <div className="bg-gray-50 p-4 rounded">
                        ...
                        <p><span className="font-medium">Air Date:</span> {sampleEpisode.airDate ? new Date(sampleEpisode.airDate).toLocaleDateString() : ''}</p>
